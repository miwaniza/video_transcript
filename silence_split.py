...
parser.add_argument('--silence-threshold', default=s.AUDIO.SILENCE_THRESHOLD, type=int,
                    help='Silence threshold (in dB)')
parser.add_argument('--silence-duration', default=s.AUDIO.SILENCE_DURATION, type=float, help='Silence duration')
...
def get_chunk_times(in_filename, silence_threshold, silence_duration, start_time=None, end_time=None):
    ...
    p = _logged_popen(
        (ffmpeg
         ...
         .output('-', format='null')
         .compile()
         ) + ['-nostats'],  # FIX_ME: use .nostats() once it's implemented in ffmpeg-python.
...
def split_audio(
        in_filename,
        out_pattern,
        silence_threshold=s.AUDIO.SILENCE_THRESHOLD,
        silence_duration=s.AUDIO.SILENCE_DURATION,
        start_time=None,
        end_time=None,
        verbose=False,
):
